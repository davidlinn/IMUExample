...
MPU9250::MPU9250() { } //do-nothing constructor

MPU9250::MPU9250(int IMUTimer)
{
  //if -1 is passed in, let the HiResTimer class assign the next available timer
  if (IMUTimer == -1)
	  timer = HiResTimer::getHiResTimer();
  else
	  timer = HiResTimer::getHiResTimer(IMUTimer);
  if (timer == NULL)
	  printf("HiResTimer%i not initialized\n",IMUTimer);
  else
	  timer->init();
}

HiResTimer* MPU9250::getIMUTimer() {
	return timer;
}
...
void MPU9250::gyromagSleep()
{
  ...
  timer->delay(.010); // Wait for all registers to reset
}
...
{
	if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01) { //Clear interrupt
...
void MPU9250::initMPU9250(uint8_t Ascale,uint8_t Gscale) //Initialization routine for MPU-9250 implemented by Kris Winer
{
...
uint8_t MPU9250::I2CWriteReadBuffer(uint8_t address, uint8_t writeVal, uint8_t *buffer, int readLength)
{
    int status;
...
void MPU9250::writeByte(uint8_t addr,uint8_t reg, uint8_t val)
{
	uint8_t buf[10];
	buf[0]=reg;
	buf[1]=val;
